...
#ifdef WIN32
    // Notify user that a blacklisted DLL was injected into XDesktop process (for example Nahimic, see GH #5573).
    // We hope that if a DLL is being injected into a XDesktop process, it happens at the very start of the application,
    // thus we shall detect them now.
    if (BlacklistedLibraryCheck::get_instance().perform_check()) {
        std::wstring text = L"Following DLLs have been injected into the XDesktop process:\n\n";
        text += BlacklistedLibraryCheck::get_instance().get_blacklisted_string();
        text += L"\n\n"
                L"XDesktop is known to not run correctly with these DLLs injected. "
                L"We suggest stopping or uninstalling these services if you experience "
                L"crashes or unexpected behaviour while using XDesktop.\n"
                L"For example, ASUS Sonic Studio injects a Nahimic driver, which makes XDesktop "
                L"to crash on a secondary monitor, see XDesktop github issue #5573";
...
extern "C" {
    // Enable printing stacktrace by default. It can be disabled by running XDesktop with "UBSAN_OPTIONS=print_stacktrace=0".
