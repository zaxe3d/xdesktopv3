        ...
        for (const auto &dir_entry : boost::filesystem::directory_iterator(dir.first)) {
            if (Slic3r::is_ini_file(dir_entry)) {
                ...
                if (!boost::filesystem::exists(idx_path)) {
                    BOOST_LOG_TRIVIAL(trace) << format("Missing index %1% when loading bundle %2%. Going to search for it in cache folder.", idx_path.string(), id);
                    idx_path = fs::path(cache_dir / (id + ".idx"));
                }
                if (!boost::filesystem::exists(idx_path)) {
                    BOOST_LOG_TRIVIAL(trace) << format("Missing index %1% when loading bundle %2%. Going to search for it in vendor folder. Is it a 3rd party profile?", idx_path.string(), id);
...
void PageMaterials::sort_list_data(PresetList* list, const std::vector<ProfilePrintData>& data)
{
    ...
    for (size_t i = 0; i < other_profiles.size(); ++i) {
        ...
        list->Check(i + zaxe_profiles.size(), other_profiles[i].second.checked);
    }
}
    ...
    : ConfigWizardPage(parent, _L("Downloads from URL"), _L("Downloads"))
{
    ...
    // append info line with link on printables.com
    {
        ...
        // TRN ConfigWizard : Downloader : %1% = "printables.com", %2% = "XDesktop"
...
bool DownloaderUtils::Worker::perform_register(const std::string& path_override/* = {}*/)
{
...
#ifdef _WIN32
    // Registry key creation for "xdesktop://" URL
    ...
    // The url will always trigger opening of xdesktop and we have to check that user has allowed it. (GUI_App::MacOpenURL is the triggered method)
#else 
...
bool ConfigWizard::priv::apply_config(AppConfig *app_config, PresetBundle *preset_bundle, const PresetUpdater *updater, bool& apply_keeped_changes)
{
    ...
    if (preferred_pt = get_preferred_printer_technology("Zaxe", bundles.zaxe_bundle());
...
{
    bool ret = false;//page_msla->any_selected();
    for (const auto& printer: pages_3rdparty)
