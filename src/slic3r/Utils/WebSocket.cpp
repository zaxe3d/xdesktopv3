...
void Websocket::run()
{
    try {
        ...
        onErrorSignal(se.what());
    } catch(beast::error_code const& ec) {
        //if (ec == beast::errc::connection_refused)
        //boost::this_thread::sleep_for(boost::chrono::seconds(30)); // delay reconnection a bit.
        onErrorSignal(ec.message());
    } catch(exception const& e) {
        onErrorSignal(e.what());
    }
}
